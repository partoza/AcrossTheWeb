...
const OptionCard = ({ selected, onClick, title, subtitle }: any) => (
  ...
  >
    {selected && (
      <div className="absolute top-4 right-4 w-5 h-5 rounded-full bg-black  text-white  flex items-center justify-center shadow-sm">
        <Check className="w-3 h-3" />
      </div>
    )}
    <span className={`font-medium text-lg leading-tight mb-1 pr-6 ${selected ? 'text-black ' : 'text-gray-700 '}`}>
      {title}
    </span>
    {subtitle && (
      <span className={`text-[13px] font-medium ${selected ? 'text-gray-600 ' : 'text-gray-400 '}`}>
        {subtitle}
      </span>
    )}
  </button>
...
const NumberStepper = ({ value, onChange, min = 1, step = 1, label }: any) => (
  <div className="flex items-center justify-between p-4 px-5 rounded-2xl border-2 border-gray-200  bg-transparent">
    ...
    <div className="flex items-center gap-3">
      <button 
        ...
        className="w-10 h-10 rounded-full bg-gray-100  flex items-center justify-center hover:bg-gray-200  active:scale-95 transition-all text-black  border border-gray-200  shadow-sm cursor-pointer"
      >
  ...
  return (
    <Dialog.Root open={true} onOpenChange={(open) => !open && onClose()}>
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed left-[50%] top-[50%] z-[100] w-full max-w-4xl translate-x-[-50%] translate-y-[-50%] overflow-hidden bg-white  shadow-2xl sm:rounded-[2rem] border border-black/5  p-0 duration-300 data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 data-[state=closed]:slide-out-to-left-1/2 data-[state=closed]:slide-out-to-top-[48%] data-[state=open]:slide-in-from-left-1/2 data-[state=open]:slide-in-from-top-[48%]">
          ...
          <div className="flex flex-col md:flex-row h-[90vh] sm:h-[650px]">
            ...
            <div className="hidden md:flex flex-col w-2/5 bg-gray-50  p-10 border-r border-gray-200  relative">
              ...
              <div className="mt-auto pt-10">
                ...
                <div className="text-3xl font-medium text-black ">{service.priceLabel}</div>
              </div>
            </div>

            {/* Right Side: Configurator */}
            <div className="flex flex-col w-full md:w-3/5 overflow-hidden">
              <div className="flex items-center justify-between p-6 sm:p-8 border-b border-gray-100  md:hidden">
                <div>
                  <Dialog.Title className="text-xl font-medium text-black  leading-tight">
                    {service.title}
                  </Dialog.Title>
                ...
                <Dialog.Close asChild>
                  <button className="rounded-full w-10 h-10 flex items-center justify-center bg-gray-100  hover:bg-gray-200  transition-colors cursor-pointer text-gray-500">
                    <X className="h-5 w-5" />
                  </button>
                </Dialog.Close>
              </div>
                ...
                <div className="space-y-10">
                  ...
                  {service.id === 'custom' && (
                    <div className="space-y-4">
                      ...
                      <textarea 
                        ...
                        className={`w-full p-5 rounded-2xl border-2 bg-transparent outline-none resize-none transition-colors text-lg ${error ? 'border-red-500 focus:border-red-600' : 'border-gray-200  focus:border-black '}`}
                      />
                      {error && (
                  ...
                  <div className="space-y-4">
                    ...
                    <textarea 
                      ...
                      className="w-full p-5 rounded-2xl border-2 border-gray-200  bg-transparent focus:border-black  outline-none resize-none transition-colors text-lg"
                    />
                  </div>
              ...
              <div className="p-6 sm:p-10 border-t border-gray-100  bg-white ">
                <div className="flex items-center justify-between mb-8">
                  ...
                  <div className="text-3xl sm:text-4xl font-medium tracking-tight text-black ">
                    {service.id === 'custom' ? 'TBD' : `₱${estimatedPrice.toLocaleString()}`}
                  </div>
                </div>
                
                <div className="flex gap-4">
                  <button onClick={onClose} className="flex-1 py-4 sm:py-5 rounded-full font-medium bg-gray-100  hover:bg-gray-200  text-black  transition-colors cursor-pointer active:scale-95 text-lg">
                    Cancel
                  </button>
                  <button onClick={handleSave} className="flex-[2] py-4 sm:py-5 rounded-full font-medium bg-black  text-white  hover:scale-[1.02] active:scale-95 transition-all shadow-xl cursor-pointer text-lg">
                    Add to Inquiry
                  </button>
                </div>
              </div>
              ...
              <Dialog.Close asChild>
                <button className="hidden md:flex absolute right-6 top-6 rounded-full w-10 h-10 items-center justify-center bg-gray-100  hover:bg-gray-200  transition-colors cursor-pointer text-gray-500">
                  <X className="h-5 w-5" />

      ...
      <div className="flex w-full lg:w-1/2 bg-white  p-4 sm:p-6 lg:p-8 h-full">
        ...
        >
          ...
          <button
            ...
            disabled={isLoading}
            className={`relative w-full group flex items-center justify-center gap-3 bg-white/10 border border-white/20 text-white px-6 py-4 rounded-[14px] font-medium text-[14px] transition-all duration-200 cursor-pointer backdrop-blur-md shadow-lg ${isLoading ? 'opacity-80 cursor-wait' : 'hover:bg-white/20 active:scale-[0.98]'}`}
          >
            <div className="bg-white rounded-full p-0.5 flex items-center justify-center w-5 h-5">
              {isLoading ? (
        ...
        >
          ...
          <div className="space-y-3">
            <button
              onClick={handleSignIn}
              disabled={isLoading}
              className={`relative w-full group flex items-center justify-center gap-3 bg-black text-white px-6 py-4 rounded-[14px] font-medium text-[14px] transition-all duration-200 cursor-pointer ${isLoading ? 'opacity-80 cursor-wait' : 'hover:shadow-xl hover:shadow-black/10 hover:-translate-y-[1px] active:translate-y-0 active:scale-[0.98]'}`}
            >
              <div className="bg-white rounded-full p-0.5 flex items-center justify-center w-5 h-5">
                {isLoading ? (
                  <Loader2 className="w-4 h-4 text-black animate-spin" />
